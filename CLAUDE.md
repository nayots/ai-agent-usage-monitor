...
| Project | What it is |
|---|---|
| `src/AiUsageMonitor.Domain` | Provider-neutral `net10.0` class library — models, states, extraction. Zero `PackageReference`, and it references nothing. |
| `src/AiUsageMonitor.Infrastructure` | Everything that touches the machine: `Providers/`, `Refresh/`, `Settings/`, `Logging/`, `Diagnostics/`, `Theming/`. References Domain. |
...
Layering follows PRD §21: `Domain/` (i.e. the `src/AiUsageMonitor.Domain` class library) is provider-neutral (models, states, extraction); `Providers/` is a folder inside `src/AiUsageMonitor.Infrastructure` holding the adapters behind `IProviderProbe`. These are **not** peer folders under one project — `Domain/` is its own standalone project precisely so it can be referenced without depending on Infrastructure or any provider adapter. Provider-specific semantics must not leak into shared or UI code.

Provider paths named elsewhere in this file (`Providers/Claude/ClaudeScopedLimits.cs`, `Providers/Codex/CodexProbe.cs`, and so on) are relative to `src/AiUsageMonitor.Infrastructure`. Earlier revisions placed `Providers/` under the POC; that has not been true since the adapters moved, and the POC now consumes them through a project reference.
...
| Kind | Path | In the repository? |
|---|---|---|
| Implementation plans | `docs/plans/` | **No** — gitignored |
| Design specs / brainstorm output | `docs/specs/` | **No** — gitignored |
| PRD, design output, pre-publication checklist | `docs/PRD.md`, `docs/design/`, `docs/pre-publication-checklist.md` | **No** — gitignored |
| Provider capability findings | `docs/provider-capability-findings.md` | Yes |
| This file, `AGENTS.md`, `docs/codex-workflow.md` | repo root and `docs/` | Yes |

**Since `4ace7ac` (2026-08-18) the process documents are local-only.** The repository went
public and is a product, not a record of how it was built. They stay on disk and stay in daily
use; they are simply not tracked. Three consequences that matter more than the tidiness does:

- **The paths did not move** — keep writing plans to `docs/plans/` and specs to `docs/specs/`
  exactly as before. Only their tracking changed.
- **The managed block below says a plan is "committed before delegating". That step is gone.**
  It cannot be committed and does not need to be: Codex reads the plan as a file on disk, which
  is all it ever read. Do not un-ignore `docs/plans/` to satisfy that sentence, and do not
  hand-edit the block to fix it — see the warning under "Two managed files" below.
- **A git worktree or a fresh clone has no plan, no spec and no PRD**, because a worktree checks
  out tracked files only. Run delegations from the primary working directory, or copy the plan
  into the worktree first. An agent that finds `docs/PRD.md` missing should recover it from
  history (`git show 09ff029:docs/PRD.md`), never regenerate it.

These paths **override the Superpowers defaults** (`docs/superpowers/plans/`, `docs/superpowers/specs/`) — those skills state that a user preference wins, and this is it. The plan path is not a preference at all but a requirement: the codex-workflow block below, and `docs/codex-workflow.md`, both name `docs/plans/<feature>.md` as the file a delegation is run against, so a plan written anywhere else is a plan Codex is never pointed at.
