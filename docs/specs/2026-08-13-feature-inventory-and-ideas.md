...
| C14 | ⬜ Open | **Provider ordering and per-provider visibility.** Drag to reorder; hide a provider outright. | Asked for, and it changes the tray glyph too — the digits come from the first visible provider's primary window. | S | §28 |
| C15 | ⬜ Open | **Per-provider refresh intervals.** Codex is a local process; Claude is a network call against an undocumented endpoint. One interval for both is the wrong shape. | Also the polite thing to do to an unofficial endpoint. | M | §28 |
